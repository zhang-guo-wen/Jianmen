...
<p align="center">
  <strong>🐳 单二进制部署 · 30MB 空闲内存 · 0.5 核 1G 即可运行 · 中小团队必备安全组件</strong>
</p>

**Jianmen**（剑门）是一个专为中小团队打造的**轻量化高性能堡垒机**。基于 Go 语言单体架构，将所有能力——SSH/SFTP 代理、多协议数据库代理、Web 管理后台、终端录像、命令审计——编译为**单个静态二进制文件**，无外部运行时依赖，空闲内存仅占 30-50MB，**在 0.5 核 1G 的容器中即可流畅运行**，是中小团队低成本构筑运维安全防线的理想选择。

### 为什么选择 Jianmen？

| 对比维度 | 传统堡垒机 | Jianmen |
|---------|-----------|---------|
| **部署复杂度** | 多组件编排，依赖 JDK/Python/数据库 | 单二进制，一键启动 |
| **资源占用** | 2-4 核 4-8G 起步 | **0.5 核 1G** 即可 |
| **启动时间** | 分钟级 | **秒级** |
| **外部依赖** | MySQL/Redis/消息队列 | 零依赖（内置 SQLite） |
| **Docker 镜像** | 数百 MB 到 GB | **Lite 版 ~15MB**（压缩后） |
| **维护成本** | 需要专职运维 | 开发人员即可维护 |
