...
- 当前真实运行路径引入 `store.Store` 接口层：数据库可用时使用 `store.DBStore`，不可用时降级为 `store.StaticAdapter`（封装原 `access.StaticStore`）。
- `DBStore` 已实现：用户认证（token / password hash）、公钥认证、Host CRUD、Target CRUD，均通过 GORM 读写 DB。
- DB 代理尚未迁移到 DBStore，仍使用 config-based 管理。
...
- OpenSSH 命令行客户端已完成一轮自动化冒烟：`ssh` 公钥认证、默认/指定资产、exec、shell、stdout/stderr、exit-status、客户端长命令中断后服务可用、审计产物通过；`sftp` 基础连接、指定资产、上传下载、目录、重命名删除、stat/chmod、中文/空格路径、100MB 大文件、100 小文件批量、客户端异常断线和审计产物通过。详见 [compatibility-matrix.md](compatibility-matrix.md)。
- Xftp、WinSCP、FileZilla 等图形 SFTP 客户端还没有正式实测矩阵。
- OpenSSH 密码登录、keyboard-interactive、真实窗口 resize、真实 Ctrl-C、TUI、Linux 目标上的 SFTP chown 等场景还需要补测。
...
- **新增 `store.Store` 接口**（`internal/store/`）：抽象了认证、Host、Target、DB Proxy 的全部 CRUD 操作。
- **新增 `store.DBStore`**：基于 GORM 的实现，覆盖 Host、HostAccount（Target）和用户认证。
- **新增 `store.StaticAdapter`**：封装 `access.StaticStore` 为 Store 接口，作为无 DB 时的降级方案。
- **`main.go`** 启动时根据 DB 是否存在自动选择 DBStore 或 StaticAdapter。
- **SSH server 已接入 RBAC**：`handleConn` 中通过 `rbacChecker.HasPermission` 校验 `session:connect` 权限。
- Admin server 中 `withAuthAndUser` 已支持 per-user token 鉴权（通过 DB `token_hash` 查询），Admin handler 重构为接受 `store.Store` 接口。

仍未完成：

- Admin API 的 RBAC 鉴权仍以全局 token fallback 为主，per-user 路径尚未全覆盖。
- Admin API 的 sessions/db audit 仍扫描 replay 文件系统，未切换到 DB 索引。
- RBAC checker 尚未接入 SFTP 文件操作授权和 DB proxy 授权。
- Web Terminal 虽已改用 `store.TargetConfig` 和 `store.ClientConfigForTarget`，但底层仍直接 `ssh.Dial`，未走统一 `sshproxy`。
...
- 完整角色和授权体系。
- 资产、资源组、会话、文件级策略。
- 元数据库作为真实运行数据源（已部分接入：Host/Target/用户认证走 DB；DB Proxy、会话索引、SFTP 文件策略尚未切换）。
- 完整审计索引、检索和录像回放。
- Web Terminal 统一审计路径。
- 主流客户端完整兼容性验证。
- 数据库完整审计与 SQL 策略。
...
| 文件操作审计 | 第一版可用 |
| 终端录像 | 第一版可用 |
| Admin API | 基础可用，已接入 Store 接口 |
| Vue Web UI | 雏形，Hosts/Sessions 相对可用 |
| Web Terminal | 已改用 Store 接口，但仍未统一 SSH proxy |
| 数据库代理 | 明文观察可用，管控不足 |
| RBAC 模型 | 有脚手架和测试 |
| RBAC 运行时执行 | SSH 已接入 session:connect 检查；SFTP/Admin/DB 尚未 |
| 元数据库产品化 | 部分接入：Store 接口 + DBStore，Host/Target/Auth 走 DB |
| 完整替代 Teleport | 未达到 |

## 结论

Jianmen 目前不是空壳，SSH/SFTP 主链路、SFTP 语义代理、基础录像和审计已经做出来了，管理 API、Web 前端和数据库代理也有可运行雏形。2026-06-25 完成了 Store 接口抽象和 DBStore / StaticAdapter 双实现，`main.go` 启动时自动选择后端；SSH server 已接入 RBAC `session:connect` 权限校验。

下一阶段应优先做五件事：

1. 完成 SSH/SFTP 主流客户端兼容性矩阵。
2. 把 DBStore 覆盖范围扩展到 DB 代理管理（当前仍是 config-only）。
3. 把 RBAC 接入 SFTP 文件操作、Admin API per-user 路径、DB proxy 连接授权。
4. 把 Web Terminal 纳入统一 SSH proxy + SessionRecorder 链路。
5. 会话审计从 replay 文件扫描切换到 DB 索引查询。
