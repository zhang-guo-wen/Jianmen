...
上游 TLS 指 `Jianmen → 实际数据库`，与客户端到 Jianmen 的网关 TLS 无关。新建数据库实例默认不启用上游 TLS，以兼容只提供 IP 或没有证书的数据库；该模式只适合受信任内网。选择 `disable` 时，测试连接与正式代理都会按该选择使用不加密的上游链路；如果 PostgreSQL 上游要求明文密码认证（例如另一台使用可选 TLS 的 Jianmen 网关），密码也会通过未加密链路发送。选择 `verify-ca` 或 `verify-full` 时，密码只会在证书验证成功的 TLS 链路中发送。MySQL `caching_sha2_password` 的完整认证不直接通过未加密链路发送明文密码；当前要求使用已验证 TLS。
