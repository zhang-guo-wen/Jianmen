...
代理。Go 服务仍是唯一控制面，负责用户身份、`host_account`、RBAC、
...
`rdp:connect` 不会自动获得剪贴板、文件或磁盘映射权限。
...
票据只能使用一次，并同时绑定用途、主机账号和连接 ID。账号有效期在会话中途
到期时，在线会话会立即终止。即使连接因 RBAC 被拒绝，也会保存一条 `denied`
审计会话。
...
仓库中的 `deploy/docker/Dockerfile` 装配一个同时包含 Jianmen 和 `guacd` 的镜像，
并基于以下不可变运行层：
...
`configs/config.docker.web-rdp.example.json`，切换为 S3 配置，通过只读挂载或密钥
管理系统提供凭据。文件系统模式的 `/app/data/objects` 位于持久化数据目录中，
不适合作为多实例共享存储；单机使用时也必须备份宿主机的 `data/`。

同一容器内两个进程使用完全相同的路径，必须与
`configs/config.docker.web-rdp.example.json` 保持一致：
