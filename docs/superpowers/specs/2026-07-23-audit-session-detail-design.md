# 审计会话 SessionID 与授权详情设计

## 背景

当前 SSH 与数据库审计列表已返回用户会话的 5 位 `session_id`，但它以内联小字形式显示在“操作用户”旁，不能点击查看授权来源。在线会话列表只返回连接本身及对应的 `audit_session_id`，没有返回可展示的用户会话 SessionID。
...
1. SSH 审计、数据库审计和在线会话列表统一增加独立的 `SessionID` 列。
2. 删除 SSH 和数据库审计“操作用户”旁原有的 `(session_id)` 内联显示。
3. SessionID 使用统一的可点击链接样式；点击后展示 UserSession 授权详情。
...
8. 三个列表共用同一套详情 API 和前端弹窗组件。

### 明确不做

1. 不修改登录审计、操作审计和 RDP 审计。
...
该方案需要在三个列表查询中重复关联和组装授权信息，增加响应体积和维护成本，因此不采用。
...
三个列表均新增独立列，列标题为 `SessionID`：

- SSH：时间、操作用户、SessionID、目标、账号、客户端 IP、状态、操作。
- 数据库：时间、操作用户、SessionID、数据库、账号、客户端 IP、状态、操作。
- 在线会话：开始时间、操作用户、SessionID、资源、协议、账号、状态、操作。

具体要求：

1. SSH 和数据库“操作用户”列只显示用户名，删除旁边原有的 `(session_id)`。
2. SessionID 存在时使用链接样式按钮显示 5 位短 ID。
3. 点击链接打开统一详情弹窗。
4. SessionID 缺失时显示 `-`，且不可点击。
5. 三个表格使用相同列宽、对齐方式和交互样式。
6. 点击 SessionID 不触发表格行的其他操作。
...
1. SSH、数据库、在线会话均出现独立 SessionID 列。
2. SSH 和数据库操作用户旁不再显示 `(session_id)`。
3. 有 SessionID 时可点击，无 SessionID 时显示 `-`。
4. 三个列表点击后打开同一个详情组件。
...
1. SSH、数据库和在线会话分别有独立的 SessionID 列。
2. SSH、数据库操作用户旁不再显示 SessionID。
3. 三个列表中的有效 SessionID 都可点击并打开统一详情弹窗。
