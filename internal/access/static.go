...
func (s *StaticStore) DatabaseInstances() []DatabaseInstanceView {
	...
	for _, inst := range instances {
		...
		views = append(views, s.databaseInstanceView(inst, count))
	}
	return views
}
...
func (s *StaticStore) databaseInstanceView(inst model.DatabaseInstance, accountCount int64) DatabaseInstanceView {
	status := inst.Status
	if status == "" {
		status = "active"
	}
