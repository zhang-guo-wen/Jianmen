...
// SQLStreamRedactor removes literal values without retaining the full SQL.
// It deliberately treats both quote styles as sensitive to preserve existing behavior.
type SQLStreamRedactor struct {
...
func NewSQLStreamRedactor(write func([]byte) error) *SQLStreamRedactor {
	return &SQLStreamRedactor{write: write}
}

func (r *SQLStreamRedactor) Write(data []byte) error {
	if r == nil || len(data) == 0 {
		return nil
	}
	if r.write == nil {
	...
	for index := 0; index < len(data); {
		...
		case databaseSQLRedactDollarCandidate:
			...
			case len(r.dollarTag) == 1 && isIdentifierStart(value),
				len(r.dollarTag) > 1 && isIdentifierPart(value):
				if len(r.dollarTag) >= maxDollarTagBytes {
					output.Write(r.dollarTag)
		...
		case databaseSQLRedactIdentifier:
			if isIdentifierPart(value) && len(r.identifier) < 256 {
				r.identifier = append(r.identifier, value)
			} else {
				sensitive := isDatabaseSQLSensitiveAssignmentKey(r.identifier)
				if bytes.EqualFold(r.identifier, []byte("authorization")) {
					r.sensitiveParts = 2
				} else {
					r.sensitiveParts = 1
				}
		...
		case databaseSQLRedactSensitiveValue:
			if !isDatabaseSQLSensitiveValuePart(value) {
				if r.sensitiveParts > 1 && (value == ' ' || value == '\t' || value == '\r' || value == '\n') {
					r.sensitiveParts--
					output.WriteByte(value)
					r.state = databaseSQLRedactSensitiveValueStart
				} else {
...
func (r *SQLStreamRedactor) Flush() error {
...
func isDatabaseSQLSensitiveValuePart(value byte) bool {
	return isIdentifierPart(value) || value == '.' || value == '/' ||
		value == '+' || value == '-'
}
