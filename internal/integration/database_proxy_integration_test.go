	...
	for _, mysqlImage := range mysqlImages() {
		...
		t.Run(sanitizeTestName(mysqlImage), func(t *testing.T) {
			...
			if !strings.Contains(mysqlImage, ":5.7") {
				certificateDir, caFile := writeMySQLIntegrationTLSFiles(t)
				containerArgs = append([]string{
					"-v", dockerBindPath(t, certificateDir) + ":/certs:ro",
				}, containerArgs...)
			...
			instance, err := fixture.store.AddDatabaseInstance(jmstore.DatabaseInstanceInput{
				Name: "docker-mysql-" + sanitizeTestName(mysqlImage), Protocol: "mysql", Address: host, Port: port, TLSMode: tlsMode, TLSCAPEM: tlsCAPEM,
			})
			if err != nil {
			...
			gatewayAddr := startDatabaseGateway(t, fixture, "mysql", gatewayLogger).address
...
func TestDatabaseGatewayPostgresAgainstDocker(t *testing.T) {
	...
	containerID := runContainer(t, "jianmen-it-postgres",
		...
		"-e", "POSTGRES_PASSWORD="+upstreamPassword,
	...
	waitPostgres(t, upstreamAddr, upstreamUser, upstreamPassword)
	assertPostgresSCRAMConfigured(t, upstreamAddr, upstreamUser, upstreamPassword)

	fixture := newMetadataFixture(t)
	host, port := splitAddress(t, upstreamAddr)
	instance, err := fixture.store.AddDatabaseInstance(jmstore.DatabaseInstanceInput{
		Name: "docker-postgres", Protocol: "postgres", Address: host, Port: port, TLSMode: "disable",
	})
	if err != nil {
	...
	wrongCADSN := fmt.Sprintf("postgres://%s:%s@%s/%s?sslmode=verify-full&sslrootcert=%s",
		url.QueryEscape(compactUsername),
		url.QueryEscape(integrationPassword),
		gateway.address,
		upstreamUser,
		url.QueryEscape(wrongCAFile),
	)
	assertPostgresGatewayTLSFailure(t, wrongCADSN, "wrong CA", postgresTLSUnknownAuthority)

	_, gatewayPort := splitAddress(t, gateway.address)
	wrongHostnameDSN := fmt.Sprintf("postgres://%s:%s@localhost:%d/%s?sslmode=verify-full&sslrootcert=%s",
		url.QueryEscape(compactUsername),
		url.QueryEscape(integrationPassword),
		gatewayPort,
		upstreamUser,
...
func assertMySQLFullAuthObserved(t *testing.T, logs, password string) {
	t.Helper()
	if !strings.Contains(logs, `"event":"caching_sha2_full_auth"`) {
		t.Fatalf("MySQL 8 full authentication was not observed: %s", logs)
	}
