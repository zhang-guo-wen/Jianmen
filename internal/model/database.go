...
type DatabaseInstance struct {
	...
	Name          string `gorm:"uniqueIndex:idx_database_instances_name_active,priority:1;size:255;not null" json:"name"`
...
type DatabaseAccount struct {
	ID                      string         `gorm:"primaryKey;size:64" json:"id"`
	InstanceID              string         `gorm:"index;index:idx_database_accounts_instance_username,priority:1;uniqueIndex:idx_dba_instance_username_active,priority:1;index:idx_database_accounts_instance_status,priority:1;size:64;not null" json:"instance_id"`
	UniqueName              string         `gorm:"uniqueIndex:idx_dba_unique_name_active,priority:1;size:128;not null" json:"unique_name"`
	Username                string         `gorm:"index:idx_database_accounts_instance_username,priority:2;uniqueIndex:idx_dba_instance_username_active,priority:2;size:128;not null" json:"username"`
	...
	ResourceID              string         `gorm:"uniqueIndex:idx_dba_resource_id_active,priority:1;size:4" json:"resource_id"`
...
func (m *DatabaseAccount) BeforeCreate(tx *gorm.DB) error {
	if err := m.FullAudit.BeforeCreate(tx); err != nil {
		return err
	}
	return ensureID(&m.ID)
