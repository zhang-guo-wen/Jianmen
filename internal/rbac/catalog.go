...
var permissionPages = []PermissionPageDefinition{
	page("quickConnect", "快速连接", "/quick-connect", 10,
		...
		action(ActionRDPConnect, "RDP 连接", "通过堡垒机建立 Windows 远程桌面会话", model.ResourceTypeHostAccount),
		action(ActionRDPClipboardRead, "读取远端剪贴板", "允许 RDP 会话读取远端剪贴板内容", model.ResourceTypeHostAccount),
		action(ActionRDPClipboardWrite, "写入远端剪贴板", "允许 RDP 会话向远端剪贴板写入内容", model.ResourceTypeHostAccount),
		action(ActionRDPFileUpload, "上传文件", "允许通过已授权的 RDP 映射盘向远端上传文件", model.ResourceTypeHostAccount),
		action(ActionRDPFileDownload, "下载文件", "允许通过已授权的 RDP 映射盘从远端下载文件", model.ResourceTypeHostAccount),
		action(ActionRDPDriveMap, "映射磁盘", "允许在 RDP 会话中映射受控磁盘", model.ResourceTypeHostAccount),
...
var actionDependencies = map[string][]string{
	...
	ActionRDPClipboardRead:      {ActionRDPConnect},
	ActionRDPClipboardWrite:     {ActionRDPConnect},
	ActionRDPFileUpload:         {ActionRDPConnect},
	ActionRDPFileDownload:       {ActionRDPConnect},
	ActionRDPDriveMap:           {ActionRDPConnect},
}
