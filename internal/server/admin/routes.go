...
func (s *Server) routes() http.Handler {
	...
	s.muxHandle(mux, "/api/ai/resources", s.withAIToken(s.withOperationAudit(s.handleAIResources)))
	s.muxHandle(mux, "/api/ai/resources/", s.withAIToken(s.withOperationAudit(s.handleAIResources)))
	...
	s.muxHandle(mux, "/api/sql-console/sessions", s.withAuthAndUser(s.handleSQLConsoleSessions))
	s.muxHandle(mux, "/api/sql-console/sessions/", s.withAuthAndUser(s.handleSQLConsoleSession))
	...
	s.muxHandle(mux, "/api/system-settings", s.withAuthAndUser(s.withSuperAdmin(s.handleSystemSettings)))
	s.muxHandle(mux, "/api/system-settings/revisions", s.withAuthAndUser(s.withSuperAdmin(s.handleSystemSettingRevisions)))
	s.muxHandle(mux, "/api/system-settings/diagnostics/", s.withAuthAndUser(s.withSuperAdmin(s.handleSystemSettingsDiagnostic)))
