...
func ParseMySQLHandshake(payload []byte) (*MySQLHandshake, error) {
	if len(payload) < 1 {
		return nil, errors.New("mysql handshake payload too short")
	}
	h := &MySQLHandshake{}
	pos := 0
	...
	if end >= len(payload) {
		return nil, errors.New("mysql handshake: unterminated server version")
	}
	h.ServerVersion = string(payload[pos:end])
	pos = end + 1

	if pos+4 > len(payload) {
		return nil, errors.New("mysql handshake: truncated connection id")
	}
	h.ConnectionID = binary.LittleEndian.Uint32(payload[pos:])
	pos += 4
	...
	if pos+2 > len(payload) {
		return nil, errors.New("mysql handshake: truncated capability lower")
	}
	capLower := binary.LittleEndian.Uint16(payload[pos:])
	pos += 2
	...
	h.StatusFlags = binary.LittleEndian.Uint16(payload[pos:])
	pos += 2

	if pos+2 > len(payload) {
		return nil, errors.New("mysql handshake: truncated capability upper")
	}
	capUpper := binary.LittleEndian.Uint16(payload[pos:])
	pos += 2
	...
	if authDataLen > 8 {
		...
		if pos+12 > len(payload) {
			return nil, errors.New("mysql handshake: truncated auth data part 2")
		}
		part2 = payload[pos : pos+12]
		pos += 12
	}

	h.AuthData = append([]byte{}, part1...)
	h.AuthData = append(h.AuthData, part2...)

	if h.CapabilityFlags&mysqlClientPluginAuth != 0 && pos < len(payload) {
		end = pos
		for end < len(payload) && payload[end] != 0 {
			end++
		}
