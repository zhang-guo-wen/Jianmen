...
$stepCount = if ($WindowsOnly) { 3 } else { 6 }
$buildTitle = if ($WindowsOnly) { "Windows local" } else { "Linux Lite + RDP" }

Write-Host "============================================"
Write-Host "  Jianmen Build ($buildTitle)"
Write-Host "============================================"
Write-Host ""

Write-Host "[1/$stepCount] Building frontend..." -ForegroundColor Cyan
...
Write-Host "[2/$stepCount] Copying frontend to embed directory..." -ForegroundColor Cyan
...
Push-Location $root
try {
    Write-Host "[3/$stepCount] Building Windows amd64..." -ForegroundColor Cyan
    ...
    if (-not $WindowsOnly) {
        ...
        try {
            ...
            Write-Host "[4/$stepCount] Building Linux amd64 Lite..." -ForegroundColor Cyan
            ...
            Write-Host "[5/$stepCount] Preparing embedded guacd runtime..." -ForegroundColor Cyan
            ...
            Write-Host "[6/$stepCount] Building Linux amd64 RDP..." -ForegroundColor Cyan
...
Write-Host "============================================"
$artifacts = if ($WindowsOnly) {
    Get-Item (Join-Path $outputDir "jianmen-windows-amd64.exe")
} else {
...
Write-Host ""
if ($WindowsOnly) {
    Write-Host "Windows local: dist/jianmen-windows-amd64.exe (Web RDP disabled by config)" -ForegroundColor Yellow
} else {
