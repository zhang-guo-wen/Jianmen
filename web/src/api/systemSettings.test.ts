...
test('system settings presents unified database entry as the default and owns the MySQL delay notice', () => {
  ...
  assert.match(source, /统一入口需要短暂等待以识别连接协议，MySQL 每次连接会增加约 200ms 建连时间。/);
});
