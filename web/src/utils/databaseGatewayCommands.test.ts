...
test('settings exposes local-only database client registration and never stores it as an account preference', () => {
  ...
  const activationDialogSource = readFileSync(new URL('../components/settings/ClientActivationDialog.vue', import.meta.url), 'utf8');
...
test('audit session IDs use registered primary links without duplicate underline styling', () => {
  const viewSource = readFileSync(new URL('../views/AuditView.vue', import.meta.url), 'utf8');
  const linkSource = readFileSync(new URL('../components/audit/AuditSessionLink.vue', import.meta.url), 'utf8');
  const mainSource = readFileSync(new URL('../main.ts', import.meta.url), 'utf8');
  const sessionLinkUsages = viewSource.match(/<AuditSessionLink\b/g) ?? [];
  const componentsStart = mainSource.indexOf('const elementComponents = [');
  const componentsEnd = mainSource.indexOf('];', componentsStart);

  assert.equal(sessionLinkUsages.length, 4);
  assert.match(linkSource, /<el-link(?=[^>]*v-if="sessionId")(?=[^>]*class="session-id-link")(?=[^>]*type="primary")[^>]*>/);
  assert.match(mainSource.slice(componentsStart, componentsEnd), /\bElLink,/);
  assert.doesNotMatch(`${viewSource}\n${linkSource}`, /--el-link-font-weight|text-decoration:\s*underline/);
});
